...
public class TradeAggregator {
    ...
    public static List<OHLCV> aggregate(List<Trade> trades, int resolution) {
        ...
        OHLCV curBucket = new OHLCV(trades.get(0).timeMillis / resolution * resolution);
        for (Trade t : trades) {
            if (t.timeMillis / resolution > curBucket.timeMillis / resolution) {
                if (curBucket.open != 0) res.add(curBucket);
                curBucket = new OHLCV(t.timeMillis / resolution * resolution);
            }
