...
public class PolygonClient {
    ...
    final static RateLimiter rateLimiter = RateLimiter.create(250);
    ...
    public static List<OHLCV> getHistMinutesForSymbol(Calendar day, String symbol) {
        ...
        String url = String.format("%s/historic/agg/minute/%s?apiKey=%s&from=%s&to=%s",
                baseUrl, symbol, apiKey, sdf.format(day.getTime()), sdf.format(nextDay.getTime()));
//        logger.debug(url);
        String content = doRequest(url);
        if (content == null)
            return null;
    ...
    public static OHLCV getHistDayForSymbol(Calendar day, String symbol) {
        ...
        String url = String.format("%s/historic/agg/day/%s?apiKey=%s&from=%s&to=%s",
                baseUrl, symbol, apiKey, sdf.format(nextDay.getTime()), sdf.format(nextDay.getTime()));
//        logger.debug(url);
        String content = doRequest(url);
        AggResponse r = gson.fromJson(content, AggResponse.class);
        if (r.ticks.size() > 0)
